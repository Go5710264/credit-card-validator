...
describe('Page start', () => { // формирование сценариев в обертке
  ...
  test('checking valid card entry', async () => {
    ...
    await page.type('.card_number', '3530202492624650'); // ввести в поле данные карты
    await page.click('.validation_check_button'); // вызвать событие click на button

    // Должны быть блоки с данными селекторами
    await page.waitForSelector('.preparation.luhnAlgorithm.hideBlock');
    await page.waitForSelector('.invalid_Luhn.luhnAlgorithm.hideBlock');
  });
